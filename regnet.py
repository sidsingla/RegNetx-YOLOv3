...
class RegNet(nn.Module):
    ...
    def __init__(self, block, layers, widths, regnetx_model="regnetx_002", num_classes=1000, 
                zero_init_residual=True, group_width=1, replace_stride_with_dilation=None,
                norm_layer=None):
        ...
        self.regnetx_model = regnetx_model
        self.regnetx_info = {}
        self.regnetx_info['regnetx_002'] = {}
        self.regnetx_info['regnetx_006'] = {}
        self.regnetx_info['regnetx_002']['route_layers'] = [[-4], [-1, 20], [-4], [-1, 8]]
        self.regnetx_info['regnetx_006']['route_layers'] = [[-4], [-1, 29], [-4], [-1, 14]]
        self.regnetx_info['regnetx_002']['route_filters'] = [368, 624, 256, 280]
        self.regnetx_info['regnetx_006']['route_filters'] = [528, 784, 256, 368]

        self.regnetx_model_info = self.regnetx_info[self.regnetx_model]

        self.module_defs = parse_model_config('config/yolov3_part.cfg')
        self.module_list = create_modules(self.module_defs, route_filters=self.regnetx_model_info['route_filters'])
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                x = torch.cat([layer_outputs[layer_i] for layer_i in self.regnetx_model_info['route_layers'][idx]], 1)
                idx += 1
            elif module_def["type"] == "shortcut":
                layer_i = int(module_def["from"])
                x = (layer_outputs[-1] + layer_outputs[layer_i])
            elif module_def["type"] == "yolo":
                x, layer_loss = module[0](x, targets, 13)
                loss += layer_loss
